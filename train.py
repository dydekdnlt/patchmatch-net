...
def test(model: torch.nn.Module, image_loader: DataLoader) -> None:
    ...
    for batch_idx, sample in enumerate(image_loader):
        ...
        new_logger.info("Iter {}/{}, test loss = {:.3f}, time = {:3f}".format(
            batch_idx + 1, num_images, loss, time.time() - start_time))
        if (batch_idx + 1) % 100 == 0:
            print("Iter {}/{}, test results = {}".format(batch_idx + 1, num_images, avg_test_scalars.mean()))
            new_logger.info("Iter {}/{}, test results = {}".format(batch_idx + 1, num_images, avg_test_scalars.mean()))
    print("final", avg_test_scalars.mean())
    new_logger.info("final {}".format(avg_test_scalars.mean()))
...
def process_samples(
    ...
    for batch_idx, sample in enumerate(image_loader):
        ...
        new_logger.info("Epoch {}/{}, Iter {}/{}, {} loss = {:.3f}, time = {:.3f}".format(
            epoch_idx + 1, args.epochs, batch_idx + 1, num_images, tag, loss, time.time() - start_time))

    print("End of processing {} samples.".format(tag))
    new_logger.info("End of processing {} samples.".format(tag))
    if tag == "test":
        save_scalars(logger, "full_test", avg_test_scalars.mean(), global_step)
        print("avg_test_scalars:", avg_test_scalars.mean())
        new_logger.info("avg_test_scalars: {}".format(avg_test_scalars.mean()))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_path", type=str, default="./checkpoints/params_000007.ckpt", help="load a specific checkpoint for parameters")
